...
bD = createBlockDiag(D, M, w);
bD = (2*M/L).*bD;
Ase = createShiftedBlockDiag(D,M, w);
Ase = (2*M/L).*Ase;
DMass = DGmass(w,M,L);
SMass = SEmass(w,M,L);
...
figure;
hold on;
plot(x, u0_gaussian, 'DisplayName', 'Initial Condition'); 
currentPlot = plot(x, U, 'DisplayName', 'Evolution');     
legend;
xlabel('x');
ylabel('U');
title('Evolution of U over time');
